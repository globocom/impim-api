...
class ThumborUrlAsJsonpTestCase(BaseImagesAPITestCase):
    ...
    def test_givin_a_callback_name(self):
        response = self.get('/thumbor_urls', image_url='http://s.glbimg.com/myimg.jpg', callback='my_own_name')
        assert response.body.startswith('my_own_name'), 'The callback should be my_own_name but it wasn\'t'

    def test_when_an_error_occours(self):
        response = self.get('/thumbor_urls', callback='my_callback_when_error')
        assert response.code == 400, 'When an error occours the response code should be 400 but it was %d' % response.code
